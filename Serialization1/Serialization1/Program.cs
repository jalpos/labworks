...
namespace Serialization
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string[] arr = token[0].Split('/');
            string[] bbb = token[1].Split('/');

            Complex cmp = new Complex(int.Parse(arr[0]), int.Parse(arr[1]));
            Complex cmb = new Complex(int.Parse(bbb[0]), int.Parse(bbb[1]));
            result = result + cmp + cmb;
            ...
            Console.WriteLine("Обьект сериализован");
            Complex ab = formatter.Deserialize(fs) as Complex;
